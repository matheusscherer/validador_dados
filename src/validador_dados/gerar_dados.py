"""Gerador de dados sintéticos com inconsistências propositais."""
...
FILIAIS = [
    "Porto Alegre - Centro", "Porto Alegre - Zona Sul", "Eldorado do Sul",
    "Canoas", "Gravataí", "São Leopoldo",
]


def gerar_base_vendas(n_registros: int = 500) -> pd.DataFrame:
    registros = []
    ...
    df.loc[idx_data_invalida, "data_venda"] = "2019-13-45"
    ...
    df.loc[idx_calculo_errado, "valor_total"] = (
        df.loc[idx_calculo_errado, "valor_total"] + 999.99
    )

    df = df.sample(frac=1, random_state=8).reset_index(drop=True)
    df["id_venda"] = range(1, len(df) + 1)
    return df
